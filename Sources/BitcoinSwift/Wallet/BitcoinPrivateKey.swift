...
public struct BitcoinPrivateKey {
    public let data: Data
    public let network: BitcoinNetwork
    public let isPublicKeyCompressed: Bool

    public init(network: BitcoinNetwork = .testnet, isPublicKeyCompressed: Bool = true) {
    ...
    public init(data: Data, network: BitcoinNetwork = .testnet, isPublicKeyCompressed: Bool = true) {
        self.data = data
        self.network = network
        self.isPublicKeyCompressed = isPublicKeyCompressed
    }
    ...
    public func publicKey() -> BitcoinPublicKey? {
        guard let data = computePublicKeyData() else {
            return nil
        }
        return BitcoinPublicKey(bytes: data, network: network)
    }
}

extension BitcoinPrivateKey: Equatable {
    public static func == (lhs: BitcoinPrivateKey, rhs: BitcoinPrivateKey) -> Bool {
